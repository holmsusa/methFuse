...
### 1. Cluster
In the first step, 'fuse.cluster()' is applied on the count matrices. This performs a hierarchical clustering of closest neighbors, and outputs a clustering tree of class \code{hclust}.
...
- 'labels': Labels of form \code{chr.pos}
- 'call': \code{fuse.cluster(K0, K1)}
- 'method': \code{fuse}
- 'dist.method': \code{fuse}
...
The tree can then be cut using 'fuse.cut.tree()', (or 'cutree' from package \code{stats}).
